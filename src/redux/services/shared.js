...
export const baseQuery = fetchBaseQuery({
  ...
  prepareHeaders: (headers, { getState }) => {
    const authorization = getState().authStore.token;
    if (authorization) {
...
export const baseQueryWithReauth = async (args, api, extraOptions) => {
  ...
  const authorization = api.getState()?.authStore.token;
  if (result.error && result.error.status === 401 && !isEmpty(authorization)) {
    localStorage.clear();
